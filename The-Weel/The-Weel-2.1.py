...
def en():
    ...
    escala = input(
        '\033[1;92m[!] Choose a scale [ex: "1 to 15" = 1:15] [!] : ')
...
def pt():
    ...
    escala = input(
        '\033[1;92m[!] Escolha uma escala [ex: "1 a 15" = 1:15] [!] : ')
    start = int(escala.split(':')[0])
    final = int(escala.split(':')[1])
    ...
    if personal_list == 'Y':
        ...
        while moreinfo == 'Y':
            str(info.append(input(f"\n\033[1;96m[*] Iinformação {num} : ")))
            linha()
            moreinfo = str(
                input("\n\033[1;92m[*] quer adicionar mais informação? [Y/N] [*] : ")).upper()
            linha()
            num = num + 1
    ...
    print("\033[1;32m [!] A LISTA ESTA PRONTA [!]")
    print("\033[1;32mO O Processo demorou: ", now-then, " Segundos.")
